...
class MTSSB_Mail
{
	...
	public function templateMail($to, $subject, $body, $from='', $header=array())
	{
        ...
        if (empty($from)) {
            $from = sprintf("%s <%s>", $this->shop['name'], $this->shop['email']);
        }

		$headers[] = sprintf('From: %s', $from);

		if (!empty($header['cc'])) {
			$headers[] = sprintf('Cc: %s', $header['cc']);
		}
		if (!empty($header['bcc'])) {
			$headers[] = sprintf('Bcc: %s', $header['bcc']);
		}
    ...
    {
        // 件名    
        $subject = apply_filters('mtssb_mail_booking_subject',
            $params['title'],
            array('article_id' => $params['article_id'], 'receiver' => $params['receiver'])
        );

        // 前文
        $header = apply_filters('mtssb_mail_booking_header',
            $params['header'],
            array('article_id' => $params['article_id'], 'receiver' => $params['receiver'])
        );

        // 後文
        $footer = apply_filters('mtssb_mail_booking_footer',
            $params['footer'],
            array('article_id' => $params['article_id'], 'receiver' => $params['receiver'])
        );
        ...
        // 送信元情報
        $from = apply_filters('mtssb_mail_booking_from',
            $this->fromshop,
            array('article_id' => $params['article_id'], 'receiver' => $params['receiver'])
        );

        return wp_mail($address, $subject, $body, $from);
    }
	...
	public function setTempVar($article=null, $booking=null)
	{
		...
		if ($booking) {
			$bVar = array(
				...
				'%CLIENT_COMPANY%' => $booking['client']['company'],
				'%CLIENT_NAME%' => $booking['client']['name'],
				'%CLIENT_EMAIL%' => $booking['client']['email'],
				'%CLIENT_POSTCODE%' => $booking['client']['postcode'],
				'%CLIENT_ADDRESS1%' => $booking['client']['address1'],
				'%CLIENT_ADDRESS2%' => $booking['client']['address2'],
				'%CLIENT_ADULT%' => $booking['client']['adult'],
				'%CLIENT_CHILD%' => $booking['client']['child'],
				'%CLIENT_BABY%' => $booking['client']['baby'],
			);
		}

		$sVar = $this->_shopTempVar();
			...
			switch ($cname) {
				...
				case 'birthday' :
					$body .= apply_filters('booking_form_birthday', '生年月日', 'mail') . "："
					 . apply_filters('booking_form_birthday_date', $client['birthday']->get_date('j'), $client['birthday']->get_date()) . "\n";
					break;
				case 'gender' :
	...
	protected function _bill_content($opflag) {
		...
		if (0 < $charge['tax_notation']) {
			$check .= "合計　" . $oBooking->money_format($bill->get_total()) . "\n"
				. ($charge['tax_notation'] == 1 ? '内' : '') . "消費税({$bill->tax}%)　"
				. $oBooking->money_format($bill->get_amount_tax($charge['tax_notation'] == 1)) . "\n";
		}

		// 総合計表示
		$check .= "総合計　" . $oBooking->money_format($bill->get_total() + ($charge['tax_notation'] == 2 ? $bill->get_amount_tax() : 0)) . "\n";
