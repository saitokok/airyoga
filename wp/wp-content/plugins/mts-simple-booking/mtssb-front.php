...
<?php
	...
	private function _reservation_of_the_day($thetime, $params)
    {
		...
		if (isset($this->reserved[$thetime][$article_id])) {
			$reserved = $this->reserved[$thetime][$article_id];
		...
		if ($link) {
			if ($skip) {
				$linkurl = esc_url(add_query_arg(array('aid' => $article_id, 'utm' => $thetime + $timetable[0]), $this->form_link));
			} else {
				$arg = array('aid' => $article_id, 'ymd' => $thetime) + (empty($cid) ? array() : array('cid' => $cid));
