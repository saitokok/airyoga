...
class MtssbCalendarView
{
    ...
    public function pagination($params, MtssbCalendar $calendar)
    {
        ...
        $prev_title = esc_html(
            apply_filters('mtssb_prev_pagination', date_i18n(__('F, Y'), $prevtime), array('prev' => $prevtime, 'cid' => $cid))
        );
        $prev_arg = array('ym' => date_i18n('Y-n', $prevtime)) + (empty($cid) ? array() : array('cid' => $cid));

        $nexttime = mktime(0, 0, 0, $month + 1, 1, $year);
        $next_title = esc_html(
            apply_filters('mtssb_next_pagination', date_i18n(__('F, Y'), $nexttime), array('next' => $nexttime, 'cid' => $cid))
        );
        $next_arg = array('ym' => date_i18n('Y-n', $nexttime)) + (empty($cid) ? array() : array('cid' => $cid));
...
?>
        <div class="monthly-prev-next">
            <div class="monthly-prev"><?php if ($calendar->monthTime <= $prevtime) {
            ...
            <div class="monthly-next"><?php if ($nexttime < $calendar->openTime) {
